...
const {answerCallbackQuery, edit} = require('../../../Helpers/index');
const {User_state, EducationActivity, PropertyLesson} = require('../../../../sequelize');

module.exports = {
    function: async (ctx, bot, unmarkLessonId) => {
        try {
            if (ctx.user.data.state.action === 'addTeacher' && ctx.user.data.state.param === 'specialization') {
                const index = ctx.user.data.state.data.specialization.indexOf(parseInt(unmarkLessonId));
                if (index > -1) {
                    ctx.user.data.state.data.specialization.splice(index, 1);
                }
    ...
    callBack: 'UnmarkASubject_number',
    type: 'regEx',
    regEx: /^UnmarkASubject_[0-9]+$/,
    extract: 'UnmarkASubject_',
    isWork: true,
